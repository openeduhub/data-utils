...
It may be useful to sort the data points according to some metric, for example such that all data that has been editorially confirmed is first. For this ~data_utils.default_pipelines.data~ provides the ~sorted_by_indices~ helper function:
...
data_sorted = sorted_by_indices(data, sort_indices)
#+end_src
